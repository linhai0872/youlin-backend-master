...
import com.linhai.youlin.common.DeleteRequest;
...
import com.linhai.youlin.model.request.*;
...
public class TeamController {
    ...
    public BaseResponse<List<TeamMemberVO>> listTeamMembers(@PathVariable Long teamId, HttpServletRequest request) {
        if (teamId == null || teamId <= 0) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }
        User loginUser = userService.getLoginUser(request);
        List<TeamMemberVO> teamMembers = teamService.listTeamMembers(teamId, loginUser);
        return ResultUtils.success(teamMembers);
    }

    // 在 TeamController.java 中添加
    @PostMapping("/kick")
    public BaseResponse<Boolean> kickMember(@RequestBody TeamKickRequest kickRequest, HttpServletRequest request) {
        if (kickRequest == null || kickRequest.getTeamId() == null || kickRequest.getKickedUserId() == null) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }
        User loginUser = userService.getLoginUser(request);
        boolean result = teamService.kickMember(kickRequest, loginUser);
        return ResultUtils.success(result);
    }
